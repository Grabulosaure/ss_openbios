...
static volatile int uu,vv;

// Flush cache for non coherent DMA
...
do_command(esp_private_t *esp, sd_private_t *sd, int cmdlen, int replylen)
{
    ...
    while ((esp->espdma.regs->cond_reg & DMA_HNDL_INTR) == 0) { 
        timeout++;
        if (timeout>100000) {
           esp->espdma.regs->cond_reg = DMA_RST_SCSI;
    ...
    (void)esp->ll->regs[ESP_FDATA]; // STATUS
    (void)esp->ll->regs[ESP_FDATA]; // MSGIN
    // Clear interrupts to avoid guests seeing spurious interrupts
    (void)esp->ll->regs[ESP_INTRPT];
...
// Flush cache for non coherent DMA
static void
cache_flush(unsigned char *a,unsigned l)
{
 int i;
 unsigned addr;
 l=(l+4) & ~3;
 //Flush combined I&D : Context : 0x13
 for (i=0;i<=l;i+=4) {
    addr=((unsigned)a)+i;
	__asm__ __volatile__("sta %%g0,[%0] %1\n\t" :  :
	             "r" (addr),
...
static int
ob_sd_read_sector(esp_private_t *esp, sd_private_t *sd, int offset, int num)
{
    ...
    if (do_command(esp, sd, 11, sd->bs * num))
        return 0;
    /*
...
ob_sd_read_blocks(sd_private_t **sd)
{
    ...
    ucell blk = POP();
    char *dest = (char*)POP();
    int pos, spb, sect_offset, sect_num;
            ...
            struct esp_dma *espdma)
{
    ...
    espdma->regs->cond_reg = DMA_RST_SCSI;
    espdma->regs->cond_reg = 0;
    DPRINTF("dma1: ");
            ...
            unsigned long dmaoffset)
{
    ...
    // SCSI reset
    esp->ll->regs[ESP_CMD] = ESP_CMD_RS;
    esp->ll->regs[ESP_CMD] = ESP_CMD_NULL;
