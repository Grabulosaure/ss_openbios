...
extern uint16_t video_type; // 0=TCX 1=CG3

static void ob_cg3_init(unsigned int slot, const char *path)
{
    char buf[6];
    /* Make the sbus node the current instance and active package for probing */
    feval("active-package my-self");
    ...
    push_str(buf);
    fword("set-args");
    feval("['] cgthree-driver-fcode 2 cells + 1 byte-load");
    fword("finish-device");
...
sbus_probe_slot_ss5(unsigned int slot, uint64_t base)
{
    ...
    case 3: // SUNW,tcx
        if (video_type)
            ob_cg3_init(slot, "/iommu/sbus/SUNW,cgthree");
        else
